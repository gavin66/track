DELIMITER //
DROP PROCEDURE IF EXISTS miniappTrack_Share_addColumn_sr;
CREATE PROCEDURE miniappTrack_Share_addColumn_sr()
  BEGIN
    DECLARE v_accountId varchar(256);
    DECLARE v_tableName varchar(256);
    DECLARE done bool DEFAULT FALSE ;
    DECLARE Cursor_wx_page CURSOR FOR (SELECT `accountId`,`tableName` FROM kmsocial_third_data.tableInfo where `tableName` like 'wx_share%');
